...
/**
 * This class implements the behaviour expected from the BITS system as required
 * for 6COM1037 Referred Cwk - June 2019 and specified in the Management
 * interface
 *
 * @author - add your name and SRN here
 * @author - if working as a pair, add your partner's name & SRN here (or leave
 * blank, if working individually)
...
//**************** BITS ************************** 
    /**
     * Constructor requires the name of the trainee manager and initial _budget.
     * Staff and jobs are also set up, with all staff set to "available" for
     * hire.
     *
    ...
    /**
     * Returns a String representation of the state of the project, including
     * the name of the manager, state of the project account, whether overdrawn
     * or not, the staff in the team (or, "No staff" if team is empty)
     *
     * @return a String representation of the state of the project, including
     * the name of the manager, state of the project account, whether overdrawn
     * or not, and the staff currently in the team,(or, "No staff" if team is
     * empty)
     *
     */
    ...
    /**
     * Returns true if project account <=0 and the team has no staff who can
     * leave. @returns true if project account <=0 and the
     *
     * team has no staff who can leave.
     */
...
//*********************** All Staff *************************    
    /**
     * Returns details of a staff member with the given name, (staff may be in
     * or out of the team)
     *
     * @param name the name of the required staff member
     * @return details of a staff member with the name specified in the
     * parameter
     *
     */
        ...
        for (Staff s : _staffToHire.values()) {
            if (staffExists(name)) {
                staffDetails.append("Staff name: " + s.getUName() + " "
                        ...
                        + "State:" + s.getState());
                if (s instanceof Analyst) {
                    staffDetails.append(" " + ((Analyst) s).isProgrammer() + " " + "Analyst");
                }
                if (s instanceof Technician) {
                    staffDetails.append(" " + ((Technician) s).isIsCiscoo() + " " + "Technician");
                }
                if (s instanceof Programmer) {
                    staffDetails.append(" " + "Programmer");
                }
    ...
    // ***************** Team Staff ************************   
    /**
     * Allows staff to be added to the team, if there is enough money in the
     * account for the retainer.The hired staff member's state is set to
     * "working" and their retainer is deducted from the project account. Return
     * the result of the hire; all messages should include the staff name and
     * state of the project account
     *
     * @param name is the name of the staff member
     * @return "Not found" if staff not found, "Already hired" if staff is
     * already hired, "Not enough money" if not enough money in the
     * account,"Hired" if staff are hired.All messages should include the staff
     * name and state of the project account
     *
     */
    ...
    /**
     * Returns true if the staff with the specified name is in the team, false
     * otherwise.
     *
     * @param name is the name of the staff
     * @return true if the staff with the name is in the team, false otherwise.
     *
     */
...
// ***************** Simulation ************************ 
    /**
     * Retrieves the job with the job reference number, or returns "No such
     * job".If job exists, finds a staff member in the team who can do the
     * job.The results of doing a job will be one of the following: "No such
     * Job" - with no further action taken." Job completed by..." - add the cost
     * of the job to account and include name of staff, - Job not completed as
     * no staff available - deduct job penalty from account, -Job not completed
     * due to staff inexperience - deduct penalty from the account. If a job is
     * not completed and the project account becomes negative, add "Project is
     * overdrawn" to the output.
     *
    ...
    public int getPenalty(int jobNo) {
        ...
        int result = -1;
        if (_jobLogic.jobExists(jobNo)) {

            for (Job job : _allJobs.values()) {
                result = job.getPenalty();
            }
        }
        return result;
    }
    ...
    public String getExperiencedStaff(int requiredLevel) {
        ...
        StringBuilder qualifiedStaff = new StringBuilder();

        
        for (Staff staff : _staffToHire.values()) {

            if (staff.getExperience() >= requiredLevel){
            qualifiedStaff.append("Staff name: " + staff.getUName() + " "
                    + "Experience level: " + staff.getExperience() + " "
                    + "Retainer: " + staff.getRetainer() + " "
                    + "Hourly rate: " + staff.getRate() + " "
                    + "State: " + staff.getState().toString() + "\n");
            }
        }
        
        return qualifiedStaff.toString();
    }
